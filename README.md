...
## Running locally
You can use micronets-mso-portal-stub to test DPP onboarding by defining the gateway IP address you want to use for onboarding.
`export gateway="192.168.0.73"`

This will send gateway REST messages directly to the gateway instead of using the regular micronets backend.
You can also send curl requests to micronets-mso-portal-stub to change the PSK for the device (autogenerated) or delete the device

```
curl -X POST http://localhost:3010/dpp/updatePSK
curl -X POST http://localhost:3010/dpp/delete
```

The same deviceID and PSK are always used. `updatePSK` does a rotate on the PSK string each time it is called.
